...
export default function SolicitarViaje() {
  ...
  const handleRutaChange = async (e) => {
    ...
    try {
      ...
      if (horariosList.length > 0) {
        setFormData((prev) => ({
          ...prev,
          horarioId: String(horariosList[0].id),
        }));
      }
    } catch {
      setHorarios([]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.precioEstimado)
      payload.precioEstimado = Number(formData.precioEstimado);
            ...
            <form onSubmit={handleSubmit}>
              ...
              {formData.horarioId && horarios.length > 0 && (
                <div className="form-group">
                  ...
                  >
                    {horarios
                      .find((h) => String(h.id) === formData.horarioId)
                      ?.horaSalida?.slice(0, 5)}
                    {horarios.find((h) => String(h.id) === formData.horarioId)
                      ?.frecuenciaMinutos
