...
import { obtenerEstadoId, obtenerEstadoColor, obtenerNombrePersona, textoCuposPasajero, horarioLabel } from "./viajesHelpers";
            ...
            {(() => {
              ...
              return conCupo.length === 0 ? (
                <div className="dashboard-card" style={{ textAlign: "center", padding: "2rem" }}>
                  ...
                  <p style={{ color: "#6b7280" }}>No hay viajes disponibles con cupo libre</p>
                </div>
              ) : (
                    ...
                    <div key={viaje.id} className="dashboard-card" style={{ padding: 0, overflow: "hidden" }}>
                      ...
                      <div style={{ padding: "0.6rem 1rem", background: "#f9fafb", borderTop: "1px solid #e5e7eb" }}>
                        <button
                          ...
                          style={{ width: "100%", fontSize: "0.85rem", display: "flex", alignItems: "center", justifyContent: "center", gap: "0.3rem", opacity: tieneViajeActivo ? 0.5 : 1 }}
                          disabled={joiningId === viaje.id || tieneViajeActivo}
                          title={tieneViajeActivo ? "Ya tienes un viaje activo" : ""}
                          onClick={() => handleViajar(viaje.id)}
                        >
                          {tieneViajeActivo ? "Ya tienes un viaje activo" : joiningId === viaje.id ? "Uniendo..." : <>Viajar <Users size={13} /></>}
                        </button>
                      </div>
                    </div>
                  ))}
                    ...
                    return (
                      <div key={viaje.id} className="dashboard-card" style={{ padding: 0, overflow: "hidden" }}>
                        ...
                        <div style={{ padding: "0.6rem 1rem", background: "#f9fafb", borderTop: "1px solid #e5e7eb", display: "flex", gap: "0.4rem" }}>
                          ...
                          <button
                            className="button button-outline"
                            style={{ flex: 1, fontSize: "0.8rem", display: "flex", alignItems: "center", justifyContent: "center", gap: "0.25rem", borderColor: "#ef4444", color: "#ef4444" }}
                            disabled={leavingId === viaje.id}
                            onClick={() => handleBajarse(viaje.id)}
                          >
                            {leavingId === viaje.id ? "Saliendo..." : <><LogOut size={13} /> Bajarme</>}
                          </button>
                        </div>
                      </div>
                    );
            ...
            <div className="dashboard-section" style={{ marginTop: "2rem" }}>
              ...
              <div style={{ display: "grid", gap: "0.75rem", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))" }}>
                {misViajes.filter((v) => {
                  ...
                  const estadoId = obtenerEstadoId(viaje);
                  return (
                    <div key={viaje.id} className="dashboard-card" style={{ padding: 0, overflow: "hidden", opacity: 0.7 }}>
                      <div style={{ padding: "1rem 1rem 0 1rem" }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "0.4rem" }}>
                          <h3 style={{ fontSize: "0.95rem", fontWeight: 700, color: "#1f2937", margin: 0 }}>
                            {viaje.ruta?.nombre || "Ruta sin nombre"}
